	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jiankangdangan?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.xuexing}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">药物过敏</div>
						<div class="text "  >{{detail.yaowuguomin}}</div>
					</div>
					<div class="item">
						<div class="lable">家族病史</div>
						<div class="text "  >{{detail.jiazubingshi}}</div>
					</div>
					<div class="item">
						<div class="lable">既往病史</div>
						<div class="text "  >{{detail.jiwangbingshi}}</div>
					</div>
					<div class="item">
						<div class="lable">住院记录</div>
						<div class="text "  >{{detail.zhuyuanjilu}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jiankangdangan','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jiankangdangan','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/jiankangdangan', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jiankangdanganAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此健康档案？') .then(_ => {
					this.$http.post('jiankangdangan/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
