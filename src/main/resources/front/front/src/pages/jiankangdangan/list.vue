			...
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					...
					<el-date-picker
						...
						type="daterange"
						range-separator="至"
						start-placeholder="登记时间起始"
						end-placeholder="登记时间结束"
				...
				<el-button class="list-add-btn" v-if="btnAuth('jiankangdangan','新增')" type="primary" @click="add('/index/jiankangdanganAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						...
						<div class="name">年龄:{{item.nianling}}</div>
						<div class="time_item">
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.timeRange&&this.timeRange.length > 0) {
					searchWhere.dengjishijianstart = this.timeRange[0];
					searchWhere.dengjishijianend = this.timeRange[1];
				}
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`jiankangdangan/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jiankangdanganDetail', query: params});
			},
