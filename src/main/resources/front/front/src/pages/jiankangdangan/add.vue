<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="血型" prop="xuexing">
				<el-select v-model="ruleForm.xuexing" placeholder="请选择血型" :disabled=" false  ||ro.xuexing" >
					<el-option
			...
			<el-form-item class="add-item" label="药物过敏" prop="yaowuguomin">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="药物过敏"
			...
			<el-form-item class="add-item" label="家族病史" prop="jiazubingshi">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="家族病史"
			...
			<el-form-item class="add-item" label="既往病史" prop="jiwangbingshi">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="既往病史"
			...
			<el-form-item class="add-item" label="住院记录" prop="zhuyuanjilu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="住院记录"
					v-model="ruleForm.zhuyuanjilu">
					</el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='zhuyuanjilu'){
							this.ruleForm.zhuyuanjilu = obj[o];
							this.ro.zhuyuanjilu = true;
							continue;
						}
						if(o=='touxiang'){
				...
				this.xuexingOptions = "A,B,AB,O".split(',')
			...
			// 多级联动参数
			info() {
				this.$http.get(`jiankangdangan/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jiankangdangan/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
