	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jiankangyisheng?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">专业擅长</div>
						<div class="text "  >{{detail.zhuanyeshanzhang}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jiankangyisheng','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jiankangyisheng','删除')" @click="delClick">删除</el-button>
						<el-button class="chatBtn" v-if="btnAuth('jiankangyisheng','私聊')&&detail.id!=mid" @click="chatClick">私聊</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="医生简介" name="first">
					<div class="ql-snow ql-editor" v-html="detail.yishengjianjie"></div>
				</el-tab-pane>
			</el-tabs>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(res.data.data.yishenggonghao){
							this.fname = res.data.data.yishenggonghao
						}
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/jiankangyisheng', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jiankangyishengAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此健康医生？') .then(_ => {
					this.$http.post('jiankangyisheng/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
