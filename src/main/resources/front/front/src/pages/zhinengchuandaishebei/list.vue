		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					<div class="lable">设备名称：</div>
					<el-input v-model="formSearch.shebeimingcheng" placeholder="设备名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">设备类型：</div>
					<el-input v-model="formSearch.shebeileixing" placeholder="设备类型" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
				...
				<el-button class="list-add-btn" v-if="btnAuth('zhinengchuandaishebei','新增')" type="primary" @click="add('/index/zhinengchuandaishebeiAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<img class="image" @click.stop="imgPreView(item.shebeitupian)" v-if="item.shebeitupian && item.shebeitupian.substr(0,4)=='http'&& item.shebeitupian.split(',w').length>1" :src="item.shebeitupian" />
						<img class="image" @click.stop="imgPreView(item.shebeitupian.split(',')[0])" v-else-if="item.shebeitupian && item.shebeitupian.substr(0,4)=='http'" :src="item.shebeitupian.split(',')[0]" />
						<img class="image" @click.stop="imgPreView(baseUrl + (item.shebeitupian?item.shebeitupian.split(',')[0]:''))" v-else :src="baseUrl + (item.shebeitupian?item.shebeitupian.split(',')[0]:'')" />
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.shebeileixing != '') searchWhere.shebeileixing = '%' + this.formSearch.shebeileixing + '%';
				...
				this.$http.get(`zhinengchuandaishebei/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/zhinengchuandaishebeiDetail', query: params});
			},
