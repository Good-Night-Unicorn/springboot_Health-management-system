	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/zhinengchuandaishebei?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
			...
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						...
						<div class="text "  >{{detail.shebeileixing}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">穿戴时间</div>
						<div class="text "  >{{detail.chuandaishijian}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('zhinengchuandaishebei','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('zhinengchuandaishebei','删除')" @click="delClick">删除</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.shebeitupian) {
							this.detailBanner = this.detail.shebeitupian.split(",w").length>1?[this.detail.shebeitupian]:this.detail.shebeitupian.split(',');
						}
						this.$forceUpdate();
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/zhinengchuandaishebei', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/zhinengchuandaishebeiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此智能穿戴设备？') .then(_ => {
					this.$http.post('zhinengchuandaishebei/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
