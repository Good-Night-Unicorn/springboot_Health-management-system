	...
	<div>
		...
		<div class="container">
			<el-form class='rgs-form animate__animated animate__' v-if="pageFlag=='register'" ref="registerForm" :model="registerForm" :rules="rules">
				<div class="rgs-form2">
					<div class="title">基于SpringBoot与Vue.js的健康管理系统</p></div>
					...
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="yishenggonghao">
					...
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="mima">
						<div class="label" :class="changeRules('mima')?'required':''">密码：</div>
						<el-input v-model="registerForm.mima" type="password" placeholder="请输入密码" />
					</el-form-item>
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="mima2">
						<div class="label" :class="changeRules('mima')?'required':''">确认密码：</div>
						<el-input v-model="registerForm.mima2" type="password" placeholder="请再次输入密码" />
					</el-form-item>
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="yishengxingming">
					...
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="keshi">
						<div class="label" :class="changeRules('keshi')?'required':''">科室：</div>
						<el-input v-model="registerForm.keshi"  placeholder="请输入科室" />
					</el-form-item>
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="zhicheng">
					...
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="lianxifangshi">
					...
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="zhuanyeshanzhang">
						<div class="label" :class="changeRules('zhuanyeshanzhang')?'required':''">专业擅长：</div>
						<el-input v-model="registerForm.zhuanyeshanzhang"  placeholder="请输入专业擅长" />
					</el-form-item>
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="touxiang">
						<div class="label" :class="changeRules('touxiang')?'required':''">头像：</div>
						<file-upload
							tip="点击上传头像"
							action="file/upload"
					...
					<el-form-item class="list-item" v-if="tableName=='jiankangyisheng'" prop="yishengjianjie">
						<div class="label" :class="changeRules('yishengjianjie')?'required':''">医生简介：</div>
						<editor
	...
	mounted() {
		if(this.$route.query.pageFlag=='register'){
			...
			if ('jiankangyisheng' == this.tableName) {
				...
				this.requiredRules.keshi = [{ required: true, message: '请输入科室', trigger: 'blur' }]
			}
			...
			if ('jiankangyisheng' == this.tableName) {
				this.rules.lianxifangshi = [{ required: true, validator: this.$validate.isMobileNotNull, trigger: 'blur' }];
				this.requiredRules.lianxifangshi = [{ required: true, message: '请输入联系方式', trigger: 'blur' }]
			}
			if ('jiankangyisheng' == this.tableName) {
				this.rules.zhuanyeshanzhang = [{ required: true, message: '请输入专业擅长', trigger: 'blur' }];
				this.requiredRules.zhuanyeshanzhang = [{ required: true, message: '请输入专业擅长', trigger: 'blur' }]
			}
		}
	},
    created() {
		...
		submitForm(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					...
					if(`jiankangyisheng` == this.tableName && this.registerForm.mima!=this.registerForm.mima2) {
						this.$message.error(`两次密码输入不一致`);
						return
					}
