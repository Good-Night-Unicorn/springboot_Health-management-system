	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jiankangshuju?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">身高(m)</div>
						<div class="text "  >{{detail.shengao}}</div>
					</div>
					<div class="item">
						<div class="lable">体重(kg)</div>
						<div class="text "  >{{detail.tizhong}}</div>
					</div>
					<div class="item">
						<div class="lable">血压(mmHg)</div>
						<div class="text "  >{{detail.xueya}}</div>
					</div>
					<div class="item">
						<div class="lable">血糖(mmol/L)</div>
						<div class="text "  >{{detail.xuetang}}</div>
					</div>
					<div class="item">
						<div class="lable">心率(次)</div>
						<div class="text "  >{{detail.xinlv}}</div>
					</div>
					<div class="item">
						<div class="lable">BMI</div>
						<div class="text "  >{{detail.shentizhiliang}}</div>
					</div>
					<div class="item">
						<div class="lable">运动记录</div>
						<div class="text "  >{{detail.yundongjilu}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">体检报告</div>
						<el-button class="uploadBtn" @click="download(detail.tijianbaogao )">点击下载</el-button>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jiankangshuju','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jiankangshuju','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('jiankangshuju','健康计划')" 
							@click="onAcross('jiankangjihua','','','','')" type="warning">
							健康计划
						</el-button>
					</div>
				</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/jiankangshuju', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 删除
			async delClick(){
				await this.$confirm('是否删除此健康数据？') .then(_ => {
