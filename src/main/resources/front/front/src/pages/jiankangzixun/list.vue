		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					<div class="lable">资讯标题：</div>
					<el-input v-model="formSearch.zixunbiaoti" placeholder="资讯标题" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('jiankangzixun','新增')" type="primary" @click="add('/index/jiankangzixunAdd')">
					添加
				</el-button>
			</el-form>
			<div class="select2">
			...
			<div class="list">
				...
				<div class="list1 index-pv1">
					<div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
						<img class="image" @click.stop="imgPreView(item.zixunfengmian)" v-if="item.zixunfengmian && item.zixunfengmian.substr(0,4)=='http'&& item.zixunfengmian.split(',w').length>1" :src="item.zixunfengmian" />
						<img class="image" @click.stop="imgPreView(item.zixunfengmian.split(',')[0])" v-else-if="item.zixunfengmian && item.zixunfengmian.substr(0,4)=='http'" :src="item.zixunfengmian.split(',')[0]" />
						<img class="image" @click.stop="imgPreView(baseUrl + (item.zixunfengmian?item.zixunfengmian.split(',')[0]:''))" v-else :src="baseUrl + (item.zixunfengmian?item.zixunfengmian.split(',')[0]:'')" />
						<div class="name">{{item.zixunbiaoti}}</div>
						<div class="name">{{item.zixunleixing}}</div>
						<div class="time_item">
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/zixunleixing/zixunleixing').then(res => {
				if (res.data.code == 0) {
					this.zixunleixingOptions = res.data.data;
					this.selectOptionsList.push({name:'资讯类型',list:this.zixunleixingOptions,tableName: 'zixunleixing',check: -1})
				}
			});
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.zixunleixing != '') searchWhere.zixunleixing = this.formSearch.zixunleixing;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`jiankangzixun/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/jiankangzixunDetail', query: params});
			},
