<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="健康评星" prop="jiankangpingxing">
				<el-select v-model="ruleForm.jiankangpingxing" placeholder="请选择健康评星" :disabled=" false  ||ro.jiankangpingxing" >
					<el-option
			...
			<el-form-item class="add-item" label="疾病风险" prop="jibingfengxian">
				<el-input v-model="ruleForm.jibingfengxian" 
					placeholder="疾病风险" clearable :disabled=" false  ||ro.jibingfengxian"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="健康方案" prop="jiankangfangan">
				<editor 
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`jiankangjihua/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jiankangjihua/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
