<template>
	<div class="main-content" :style='{"color":"#666","padding":"30px","fontSize":"16px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"margin":"10px 0","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">日期</label>
				...
				<el-row class="actions" :style='{"margin":"10px 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('jiankangshuju','健康数据统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 0px","fontSize":"inherit","color":"inherit","height":"34px"}'></span>
						健康数据统计
					</el-button>
				</el-row>
			</el-form>
			<div :style='{"width":"100%","padding":"0px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' prop="tijianbaogao" label="体检报告">
						<template slot-scope="scope">
							<el-button v-if="scope.row.tijianbaogao" type="text" size="small" @click="download(scope.row.tijianbaogao)">下载</el-button>
							<span v-else >无</span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('jiankangshuju','健康计划')" @click="jiankangjihuaCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<jiankangjihua-cross-add-or-update v-if="jiankangjihuaCrossAddOrUpdateFlag" :parent="this" ref="jiankangjihuaCrossaddOrUpdate"></jiankangjihua-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="yonghuxingmingChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import jiankangjihuaCrossAddOrUpdate from "../jiankangjihua/add-or-update";
	import {
		Loading
	} from 'element-ui';
	export default {
		data() {
			return {
				...
				jiankangjihuaCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			jiankangjihuaCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.jiankangjihuaCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiankangjihuaCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiankangjihuaCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var yonghuxingmingChart1 = echarts.init(document.getElementById("yonghuxingmingChart1"),'macarons');
					this.$http({
						url: `jiankangshuju/valueMul/yonghuxingming?yColumnNameMul=xueya,xuetang,xinlv,shentizhiliang`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res[3].length;i++){
								...
								xAxis4.push(res[3][i].yonghuxingming);
								yAxis4.push(parseFloat((res[3][i].total)));
								pArray4.push({
									value: parseFloat((res[3][i].total)),
									name: res[3][i].yonghuxingming
								})
							}
							var option = {};
							let titleObj = this.line.title
							titleObj.text = '健康数据统计'
							
							const legendObj = this.line.legend
							legendObj.data = [
								'血压'.split('/').length>1?'血压'.split('/')[0]:'血压',
								'血糖'.split('/').length>1?'血糖'.split('/')[0]:'血糖',
								'心率'.split('/').length>1?'心率'.split('/')[0]:'心率',
								'BMI'.split('/').length>1?'BMI'.split('/')[0]:'BMI',
							]
							...
							let seriesObj = [
								{
									data: yAxis1,
									type: '血压'.split('/').length>1?'血压'.split('/')[1]:'line',
									name: '血压'.split('/').length>1?'血压'.split('/')[0]:'血压',
								},
								{
									data: yAxis2,
									type: '血糖'.split('/').length>1?'血糖'.split('/')[1]:'line',
									name: '血糖'.split('/').length>1?'血糖'.split('/')[0]:'血糖',
								},
								{
									data: yAxis3,
									type: '心率'.split('/').length>1?'心率'.split('/')[1]:'line',
									name: '心率'.split('/').length>1?'心率'.split('/')[0]:'心率',
								},
								{
									data: yAxis4,
									type: 'BMI'.split('/').length>1?'BMI'.split('/')[1]:'line',
									name: 'BMI'.split('/').length>1?'BMI'.split('/')[0]:'BMI',
								},
							]
							...
							// 使用刚指定的配置项和数据显示图表。
							yonghuxingmingChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								yonghuxingmingChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
