	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="专业擅长" prop="zhuanyeshanzhang" >
					<el-input v-model="ruleForm.zhuanyeshanzhang" placeholder="专业擅长" clearable  :readonly="ro.zhuanyeshanzhang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="专业擅长" prop="zhuanyeshanzhang" >
			...
			<el-form-item v-else-if="ruleForm.yishengjianjie" label="医生简介" prop="yishengjianjie" >
				<span class="text ql-snow ql-editor" v-html="ruleForm.yishengjianjie"></span>
			</el-form-item>
		...
		data() {
			...
			return {
				...
				rules: {
					...
					zhuanyeshanzhang: [
						{ required: true, message: '专业擅长不能为空', trigger: 'blur' },
					],
					yishengjianjie: [
					],
					touxiang: [
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.yishengjianjie = this.ruleForm.yishengjianjie.replace(reg,'../../../springbootpfcf09i8/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `jiankangyisheng/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.jiankangyishengCrossAddOrUpdateFlag = false;
											this.parent.search();
